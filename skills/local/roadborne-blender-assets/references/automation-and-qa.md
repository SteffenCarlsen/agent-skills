...
For Roadborne, read ART-Q03 before running Blender. A background audit or export does not select
the Blender version, units, axes, GLB settings, collision rules, or navigation contract. Use the
exact owner-authorized .blend and write to a new explicit output path.
...
An exporter returning success is only one signal. Verify by importing into a clean scene and the
exact Roadborne Godot project and checking:
...
For WorldPreview integration, use a non-skipped asset import through tools/Run-WorldPreview.ps1,
then capture the selected append-only review anchor with the same renderer, FOV, viewport,
Beauty/HUD state, camera, and worktree/DLL identity as its baseline. Keep the raw Blender audit,
export hash, Godot import log, capture hash, and visual verdict as separate evidence.
